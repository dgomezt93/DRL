...
def extract_dicom_metadata(dicom_file):
    ...
    metadata.append(ds[tagModel].value)
...
def mostrarTablaResultados():
    ...
    Dosis_Entrada_np = dfResultados["Entrance Dose [mGy]"].to_numpy()  # Convert to NumPy array if needed
...
columnas = ['Archivo', 'Estudio', 'kV', 'Tiempo de Exposición mSec', 'mAs', "Organ Dose [mGy]", 'Entrance Dose [mGy]', 'Proyección', 'Espesor', 'Ánodo/Filtro', 'Fabricante','Modelo','Institución']
    ...
    try:
        ...
        if Model in T1:
            ...
            if est in unique_strings_2:
                ...
                Entrance_Dose_np = dfresultado['Entrance Dose [mGy]'].to_numpy()  # Convert to NumPy array if needed
                ...
                columnas2 = ['Estadística','Entrance Dose [mGy]']#,'1st Quartile' ,'2nd Quartile','3rd Quartile' ,'IQR' ,'Min' ,'Max' , 'KAP (Gy*cm^2)' , 'Air Kerma (mGy)' ,'Fluoroscopy time (min)' ,'Exposure images' ]#el mismo orden del arreglo con la informacion de la imagen dicom 
