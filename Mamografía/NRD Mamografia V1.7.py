    ...
    try:
        ...
        if Loc in T1:
            ...
            for est in unique_strings_2:
                ...
                #print(dfresultado)
              # Agrupar por UID y Proyección para sumar la dosis por proyección individual
                df_agrupado = df2.groupby(['UID', 'Proyección'], as_index=False)['Dosis de Entrada [mGy]'].sum()

                # Calcular la dosis promedio por proyección
                df_agrupado['Dosis Por Proyeccion'] = df_agrupado['Dosis de Entrada [mGy]'] / 2

               


                Entrance_Dose_np = df_agrupado['Dosis Por Proyeccion'].to_numpy()  # Convert to NumPy array if needed
                ...
                media_ed = np.mean(Entrance_Dose_np)  # 👈 Media con NumPy
                ...
                print("sq_ed:", sq_ed)
                print("tq_ed:", tq_ed)
                print("Promedio (media):", media_ed)
                print("max_ed:", max_ed)
                print("min_ed:", min_ed)
                print("Ran_intercuartil", RIQ_ED)

                 # Agrupar por UID y Proyección para sumar la dosis por proyección individual
                df_agrupado_DG = df2.groupby(['UID', 'Proyección'], as_index=False)['Dosis Glandular [mGy]'].sum()

                # Calcular la dosis promedio por proyección
                df_agrupado_DG['Dosis Por Proyeccion'] = df_agrupado_DG['Dosis Glandular [mGy]'] / 2

                print('Se va a imprimir df agrupado') 
                print(df_agrupado_DG)



                Organ_dose_np = df_agrupado_DG['Dosis Por Proyeccion'].to_numpy()  # Convert to NumPy array if needed
                    
                # Calculate percentiles and statistics
                fq_dg = np.percentile(Organ_dose_np, 25)
                sq_dg = np.percentile(Organ_dose_np, 50)
                tq_dg = np.percentile(Organ_dose_np, 75)
                media_dg = np.mean(Organ_dose_np)  # 👈 Media con NumPy
                ...
                dataFrame.loc[len(dataFrame)] = ['Promedio', "{:.2f}".format(media_ed), "{:.2f}".format(media_dg)]
                ...
                if carpeta_destino:
                    ...
                    # Guardar el DataFrame en el archivo seleccionado
                    df_agrupado.to_excel(ruta_DE, index=False)
...
# Calcular la dosis promedio por estudio
df_dosis_examen['Dosis Por Estudio [mGy]'] = df_dosis_examen['Dosis de Entrada [mGy]'] / 4



Dosis_Entrada_np = df_dosis_examen['Dosis Por Estudio [mGy]'].to_numpy()  # Convert to NumPy array if needed
                    
# Calculate percentiles and statistics
fq_ed = np.percentile(Dosis_Entrada_np, 25)
sq_ed = np.percentile(Dosis_Entrada_np, 50)
tq_ed = np.percentile(Dosis_Entrada_np, 75)
max_ed = np.max(Dosis_Entrada_np)
min_ed = np.min(Dosis_Entrada_np)
RIQ_DE = tq_ed - fq_ed
...
print("max_ed:", max_ed)
print("min_ed:", min_ed)
print("Ran_intercuartil", RIQ_DE)

# Agrupar por UID (examen) y sumar la dosis de todas las proyecciones
df_dosisGlandular_examen = df.groupby('UID', as_index=False)['Dosis Glandular [mGy]'].sum()

# Calcular la dosis promedio por estudio 
df_dosisGlandular_examen['Dosis Por Estudio [mGy]'] = df_dosisGlandular_examen['Dosis Glandular [mGy]'] / 4


Dosis_glandular_np = df_dosisGlandular_examen['Dosis Por Estudio [mGy]'].to_numpy()  # Convert to NumPy array if needed
                    
# Calculate percentiles and statistics
fq_dgm = np.percentile(Dosis_glandular_np, 25)
sq_dgm = np.percentile(Dosis_glandular_np, 50)
tq_dgm = np.percentile(Dosis_glandular_np, 75)
max_dgm = np.max(Dosis_glandular_np)
min_dgm = np.min(Dosis_glandular_np)
RIQ_DGM = tq_dgm - fq_dgm

print("fq_ed:", fq_dgm)
print("sq_ed:", sq_dgm)
print("tq_ed:", tq_dgm)
print("max_ed:", max_dgm)
print("min_ed:", min_dgm)
print("Ran_intercuartil", RIQ_DGM)


    

columnas3 = ['Estadística','Dosis Entrada [mGy]', 'Dosis Glandular Media [mGy]']#,'1st Quartile' ,'2nd Quartile','3rd Quartile' ,'IQR' ,'Min' ,'Max' , 'KAP (Gy*cm^2)' , 'Air Kerma (mGy)' ,'Fluoroscopy time (min)' ,'Exposure images' ]#el mismo orden del arreglo con la informacion de la imagen dicom 
#'1st Quartile', '2nd Quartile' ,'3rd Quartile' ,'IQR','Min', 'Max',
dataFrameDT = pd.DataFrame(columns=columnas3)# creo un dataframe y nombro las columnas del como sea requerido

# Append statistics to DataFrame
dataFrameDT.loc[len(dataFrameDT)] = ['1er Cuartil', "{:.2f}".format(fq_ed),"{:.2f}".format(fq_dgm)]
dataFrameDT.loc[len(dataFrameDT)] = ['2do Cuartil', "{:.2f}".format(sq_ed),"{:.2f}".format(sq_dgm)]
dataFrameDT.loc[len(dataFrameDT)] = ['3er Cuartil', "{:.2f}".format(tq_ed),"{:.2f}".format(tq_dgm)]
dataFrameDT.loc[len(dataFrameDT)] = ['Min', "{:.2f}".format(min_ed),"{:.2f}".format(min_dgm)]
dataFrameDT.loc[len(dataFrameDT)] = ['Max', "{:.2f}".format(max_ed), "{:.2f}".format(max_dgm)]
dataFrameDT.loc[len(dataFrameDT)] = ['Rango Intercuartílico ', "{:.2f}".format(RIQ_DE),"{:.2f}".format(RIQ_DGM)]
