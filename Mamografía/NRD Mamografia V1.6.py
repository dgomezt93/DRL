...
while True:
    try:
        # Normalizar texto: quitar espacios múltiples, saltos de línea, tabs y estandarizar mayúsculas
        df['Localización'] = (
            df['Localización']
            .astype(str)
            .str.replace(r'[\s\t\n\r]+', ' ', regex=True)  # reemplaza cualquier espacio, tab o salto por uno solo
            .str.strip()
            .str.title()  # convierte todo a "Estilo Título" para unificar mayúsculas/minúsculas
        )
        
        T1 = sorted(df['Localización'].unique())
        # Inicializa la variable Model con un valor por defecto
        Model = None
        layout = [
